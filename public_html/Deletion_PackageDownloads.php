<head><title>Delete package downloads</title></head>
<body>
<?php
    include 'open.php';
    ...
    if (!empty($packageName)) {
        $stmt = $conn->prepare('CALL DeletePackageDownloads(?)');
        if (!$stmt) {
        ...
        if (!($result)) {
            echo "<p style='margin: 0'>Successfully deleted downloads from package <span style='font-weight: bold'>".$packageName."</span>.</p>";
        } else {
